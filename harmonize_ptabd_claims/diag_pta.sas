...
/* variable names for 2016 forward ***/
...
%let poa=clm_poa_ind_sw;
%let medpoa=dgns_poa;
%let fst_e_dx=fst_dgns_e_cd; * dropped 2016 on;
%let e_dx=icd_dgns_e_cd; /* standard */
%let e_poa=clm_e_poa_ind_sw;
%let opv_dxn=3;
%let opvdx=rsn_visit_cd;
...
%extprocs1(ip,&stay_dt    &stay_dx    &pta_pdx    &fst_e_dx    %diagvlist(&dxv,1,&pta_dxn)      %diagvlist(&e_dx,1,&pta_edxn)
           ...
           asisnames=&dtstd,
           idvar=bene_id,claimvar=clm_id,typsfx=C,
           smp=c,clms=,begy=2015,endy=2016);
...
%extprocs1(hha,&pta_pdx    &fst_e_dx    %diagvlist(&dxv,1,&pta_dxn) clm_thru_dt clm_from_dt %diagvlist(&e_dx,1,&pta_edxn)  ,   
               &pta_pdxstd &fst_e_dxstd %diagvlist(&dxstd,1,&pta_dxn) thru_dt from_dt %diagvlist(&e_dxstd,1,&pta_edxn),
           idvar=bene_id,claimvar=clm_id,typsfx=C,
           smp=c,clms=,begy=2015,endy=2015);

%extprocs1(hha,&pta_pdx    &fst_e_dx    %diagvlist(&dxv,1,&pta_dxn) %diagvlist(&e_dx,1,&pta_edxn) ,   
               &pta_pdxstd &fst_e_dxstd %diagvlist(&dxstd,1,&pta_dxn) %diagvlist(&e_dxstd,1,&pta_edxn),
           asisnames=&dtstd,
           idvar=bene_id,claimvar=clm_id,typsfx=C,
           smp=c,clms=,begy=2016,endy=2016);
