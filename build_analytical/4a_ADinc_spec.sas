...
* PURPOSE:  Setting up the AD incident;

* INPUT: adrd_specdt_all_2002_&maxyr.; 
* OUTPUT: adrd_dxprv_specrate_any/adrd_dxprv_specrate_primary;
...
proc contents data=addrugs.ad_specdt_all_2002_&maxyr. out=contents; run;
...
data ad_cogspec;
	set addrugs.ad_specdt_all_2002_&maxyr.;
	year=year(demdx_dt);
	if bene_id ne "";
...
options obs=50;
proc print data=ad_cogspec; where taxcode ne 1; run;
options obs=max;

proc means data=ad_cogspec missing;
	class year;
	var npi hcfacode taxcode hasspec npi_hcfacode npi_taxcode npi_spec;
	output out=ad_cogspec1 (drop=_type_ _freq_) mean=;
run;
...
data addrugs.ad_dxprv_specrate_&method;
	set adbase;
...
proc freq data=addrugs.ad_dxprv_specrate_&method;
	table match spec mult_spectype/ missing;
run;

proc contents data=addrugs.ad_dxprv_specrate_&method; run;
proc sort data=addrugs.ad_dxprv_specrate_&method; by bene_id demdx_dt; run;
