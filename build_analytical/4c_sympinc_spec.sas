...
* INPUT: symp_specdt_all_2002_&maxyr.; 
* OUTPUT: symp_dxprv_specrate_any/symp_dxprv_specrate_primary;
...
proc contents data=addrugs.symp_specdt_all_2002_&maxyr. out=contents; run;
...
%macro cog_spec_ck;
data symp_cogspec;
	set addrugs.symp_specdt_all_2002_&maxyr.;
	year=year(demdx_dt);
	if bene_id ne "";
	...
	* how many records have a hcfa specialty;
	hcfacode=0;
	array hcfa [*] car_hcfaspcl: primaryhcfa:;
	hcfacode=0;
	do i=1 to dim(hcfa);
...
data symp_no_taxcode;
	set symp_cogspec;
	if taxcode ne 1;
run;

options obs=50;
proc print data=symp_cogspec; where taxcode ne 1; run;

options obs=max;

proc means data=symp_cogspec missing;
	class year;
	var npi hcfacode taxcode hasspec npi_hcfacode npi_taxcode npi_spec;
	output out=symp_cogspec1 (drop=_type_ _freq_) mean=;
run;
...
data addrugs.symp_dxprv_specrate_&method;
	set sympbase;
			...
			if match ne 1 then do i=1 to dim(&&var&i.._spec);
				if &&var&i..[i] ne "" then do;
					...
					if &&var&i.._spec[i]=1 then do;
						...
						if tax&&var&i..[i]=&tax_neuropsych then do; spec_type="s"; spec_neuropsych=1; end;
					end;
				end;
			end;
		%end;
...
proc freq data=addrugs.symp_dxprv_specrate_&method;
	table match spec mult_spectype/ missing;
run;

proc contents data=addrugs.symp_dxprv_specrate_&method; run;
proc sort data=addrugs.symp_dxprv_specrate_&method; by bene_id demdx_dt; run;
...
%cogspec_priorities(method=primary,prioritylist=car_hcfaspcl 
