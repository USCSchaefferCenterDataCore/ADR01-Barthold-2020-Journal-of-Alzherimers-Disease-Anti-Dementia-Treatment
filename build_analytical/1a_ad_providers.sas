...
%macro prov(input,ctyp,byear,eyear,provv=,byvar=,keepv=);
...
data 
	...
	%if "&ctyp"="car_" %then %do;
		set &input. (where=(find(line_dxtype,'A')));
	%end;
	%else %if "&ctyp" ne "car_" %then %do;
		set &input. (where=(find(dxtypes,'A')));
	%end;
...
data addrugs.ad_dxprv_&ctyp.&minyr._&maxyr;
	set 
...
proc freq data=addrugs.ad_dxprv_&ctyp.&minyr._&maxyr;
	table foundprv;
run;
%mend;
...
proc contents data=addrugs.ad_dxprv_hha_2002_&maxyr;
proc contents data=addrugs.ad_dxprv_ip_2002_&maxyr;
proc contents data=addrugs.ad_dxprv_op_2002_&maxyr;
proc contents data=addrugs.ad_dxprv_snf_2002_&maxyr;
proc contents data=addrugs.ad_dxprv_car_r2002_&maxyr;
proc contents data=addrugs.ad_dxprv_car_2002_&maxyr; 
run;

%macro freq(ctyp);
proc freq data=addrugs.ad_dxprv_&ctyp.2002_&maxyr;
	table foundprv;
	title3 "&ctyp match";
run;
%mend;
